...
This repository's \code{course-ci.yml} workflow runs on pushes, pull requests,
and manual dispatch. It grants read-only repository contents permission, cancels
obsolete runs for the same branch, and evaluates the course on Linux, macOS,
and Windows. Each runner installs the pinned tools, reproduces the locked
environment, prepares the notebook kernel, checks style, builds the package, and
runs all package, repository, and notebook tests. A final gate requires every
