...
\begin{notebooklink}
Lecture 10 notebook 00 develops gradient descent from scalar and vector
functions, then derives a single identity-activation neuron trained by squared
error. A real diabetes dataset connects its parameter-space path to the fitted
line in data space and verifies agreement with least squares; a narrow SQL query
makes the data-extraction boundary explicit. Later notebooks compare ridge,
lasso, coefficient stability, and packaged diagnostics.
\end{notebooklink}
