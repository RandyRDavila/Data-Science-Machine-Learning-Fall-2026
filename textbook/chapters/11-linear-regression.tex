...
For any candidate fitted vector $Av$ in the column space,
\[
\lVert y-Av\rVert_2^2
=\lVert y-A\widehat\theta\rVert_2^2
+\lVert A\widehat\theta-Av\rVert_2^2.
\]
This Pythagorean identity proves that the orthogonal projection minimizes
squared distance. It also separates unexplained residual length from the extra
error caused by choosing another vector in the same model space.
...
\begin{enumerate}
  ...
  \item Prove the Pythagorean identity for least-squares projection.
  \item Compare the assumptions needed for a point prediction, a coefficient
  confidence interval, and a causal intervention claim.
  \item Use the singular-value decomposition to explain which directions ridge
  shrinks most and why.
\end{enumerate}
